...
def main(weight_file_name, base_weight_name=None):
    ...
    joy_pub = rospy.Publisher(Config.data_collection['vehicle_control_topic2'], Control, queue_size = 10)
    joy_data = Control()
...
if __name__ == "__main__":
    try:
        if config['style_run'] is True:
            if len(sys.argv) != 3:
                exit('Usage:\n$ rosrun run_neural run_neural2.py style_weight_name base_weight_name')
