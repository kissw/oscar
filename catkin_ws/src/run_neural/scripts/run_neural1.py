...
def main(weight_file_name, base_weight_name=None):
    ...
    joy_pub = rospy.Publisher(Config.data_collection['vehicle_control_topic1'], Control, queue_size = 10)
    joy_data = Control()
...
if __name__ == "__main__":
    try:
        if config['style_run'] is True:
            if len(sys.argv) != 3:
                exit('Usage:\n$ rosrun run_neural run_neural1.py style_weight_name base_weight_name')
            main(sys.argv[1], sys.argv[2])
                
        else:
            if len(sys.argv) != 2:
                exit('Usage:\n$ rosrun run_neural run_neural1.py weight_file_name')
            main(sys.argv[1])
