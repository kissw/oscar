...
def main(data_path):
    ...
    csv_header = ['image_fname', 
                  ...
                  'pos_x', 'pos_y', 'pos_z']
    data = DriveData(csv_backup_name, csv_header)
    ...
    new_csv = []
    for i in bar(range(num_data-2)): # we don't have a title
        # print(i)
        # current_image_path = data_path + data.df.loc[i]['image_fname']
        
        # for frame in range(20):
        for n in range(10):
            frame = (random.randrange(10)+1)*2
            ...
            v = v_i / (frame+1)
            s = s_i / (frame+1)
            # print(data.df.loc[i+frame]['linux_time'], data.df.loc[i]['linux_time'])
            t = float(data.df.loc[i+frame]['linux_time']) - float(data.df.loc[i]['linux_time'])
            frame_t1 = int(frame/2)
            for f in range(frame_t1, frame+1):
                v_i_t1 += float(data.df.loc[i+f]['vel'])
                s_i_t1 += float(data.df.loc[i+f]['steering_angle'])
            # print((frame+1), frame_t1, (frame+1 - frame_t1))
            v_t1 = v_i_t1 / (frame+1 - frame_t1)
            s_t1 = s_i_t1 / (frame+1 - frame_t1)
            t_t1 = float(data.df.loc[i+frame]['linux_time']) - float(data.df.loc[i+frame_t1]['linux_time'])

            # print(frame, v, s, t, frame_t1, v_t1, s_t1, t_t1)
            ...
            new_csv.append(data.image_names[i] + ','
                        ...
                        + str(float(v)) + ',' 
                        + str(float(t)) + ','
                        + str(data.df.loc[i+frame_t1]['image_fname']) + ','
                        + str(float(s_t1)) + ','
                        + str(float(v_t1)) + ',' 
                        + str(float(t_t1)) + '\n')
