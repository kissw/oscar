...
def rebuild_data(data_path):
    ...
    rebuild_path = data_path+'rebuild'
    ...
    for i in bar(range(len(data.df))):
        if os.path.exists(data_path + data.image_names[i]):
            # 이미지 확인후 rebuild path에 이미지저장.
            shutil.copy(data_path + data.image_names[i], rebuild_path + '/' + data.image_names[i])
        else:
            print(data.image_names[i], 'is not found.')
    
###############################################################################
#
def main():
    if (len(sys.argv) != 2):
        print('Usage: \n$ python rebuild_dataset data_csv_name')
        return

    rebuild_data(sys.argv[1])
