...
encoder_inputs_img = keras.Input(shape=(160, 160, 3))
encoder_inputs_str = keras.Input(shape=(1))
encoder_inputs_vel = keras.Input(shape=(1))
encoder_inputs_time = keras.Input(shape=(1))
...
encoder = keras.Model([ encoder_inputs_img, encoder_inputs_str,
...
samples = list(zip( data.image_names, data.velocities, data.measurements, 
                    data.tar_image_names, data.tar_steering_angle, data.tar_vel, data.tar_time))
...
for image_name, velocity, measurement, tar_image_name, tar_steering_angle, tar_vel, tar_time in samples:
...
vae.fit(x_train, epochs=30, batch_size=16)
