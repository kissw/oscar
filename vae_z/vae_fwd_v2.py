...
encoder_inputs_str = tf.keras.Input(batch_shape=(batch_size*2, 1))
encoder_inputs_vel = tf.keras.Input(batch_shape=(batch_size*2, 1))
encoder_inputs_time = tf.keras.Input(batch_shape=(batch_size*2, 1))
...
bilstm = layers.Bidirectional(layers.LSTM(500, batch_size=batch_size*2, stateful=True))(latent)
...
# load weight
pretrained_path = '/home2/kdh/vae/new_dataset/vae_latent_compute/vae_b64_mse_latent_compute_ckpt.45-6152.8707.h5'
...
# validdata_path = "/home2/kdh/vae/new_dataset/2023-08-22-17-26-01/"
...
def prepare_vae_batch_samples(batch_samples, data_path):
    ...
    return images, tar_images, batch_tar_steering_angles, batch_tar_vels, batch_tar_times
...
train_data, valid_data = train_test_split(train_data, test_size=0.01, shuffle=True)
...
optimizer=tf.keras.optimizers.Adam(learning_rate=0.00005,decay=0.0001) 
...
# reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=100, min_delta=0.000005, min_lr=0.0000000001)

callbacks = []
model_ckpt_name = "/home2/kdh/vae/new_dataset/vae_latent_compute/vae_b64_mse_latent_compute"
